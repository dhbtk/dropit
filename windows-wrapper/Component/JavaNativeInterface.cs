...
namespace JNI
{
    public unsafe class JavaNativeInterface : IDisposable
    {
        ...
        public void CallMainMethod(string className, string[] args)
        {
            var javaClass = env.FindClass(className);
            var mainMethod = env.GetStaticMethodID(javaClass, "main", "([Ljava/lang/String;)V");
            var emptyArray = env.NewObjectArray(args.Length, env.FindClass("java/lang/String"), IntPtr.Zero);
            for (int i = 0; i < args.Length; i++)
            {
                IntPtr str = env.NewString(args[i], args[i].Length);
                env.SetObjectArrayElement(emptyArray, i, str);
            }
            env.CallStaticVoidMethod(javaClass, mainMethod, new JValue { l = emptyArray });
        }
